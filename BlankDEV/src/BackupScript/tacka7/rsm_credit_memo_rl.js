    ...
    function post(requestBody) {
      ...
      if (requestBody.action === 'createJournal') {
        ...
        try {
          ...
          rec.setText({
            fieldId: 'memo',
            text: cmCustomer + " / CM" + cmTranId + " / " + cmInvoice
          });

          // TODO Find a more convenient way to get this account - can be named differently in other envs... id will certainly be different
          var accResultSet = query.runSuiteQL({
            ...
            params: ["PDV ZA PREKNJIZENJE"]
          });
          var accId = accResultSet.results[0].values[0];

          // TODO Need to find more convenient way to get undef tax code (can have diff. name and id in diff. env.)
          var undefTaxCode = query.runSuiteQL({
          ...
          var undefTaxCodeId = undefTaxCode.results[0].values[0];
        ...
        try {
          ...
          for (var i = 0; i < lineCount; i++) {
            var netAmt = parseFloat(cmRec.getSublistValue({
              sublistId: 'item',
              fieldId: 'amount',
              line: i
            }));
          ...
          var domain, logoUrl, companyName, address, phone, emailUrl, webSite, accountNumber, pib, maticniBroj, city, country;
          if (subsidiaryFeatureCheck) {
            ...
            try {
              ...
              var addrSubRec = companyInfo.getSubrecord({
                fieldId: 'mainaddress'
              });
              address = addrSubRec.getValue({
                fieldId: 'addr1'
              ...
              logoUrl = ''; domain = ''; companyName = ''; address = ''; phone = ''; emailUrl = ''; webSite = '';
              pib = ''; maticniBroj = '';
          ...
          var data = {
            ...
            totalNetAmount: formatCurrency(requestBody.data.netAmount.toFixed(2)),
            totalTaxAmount: formatCurrency(requestBody.data.taxAmount.toFixed(2)),
            totalGrossAmount: formatCurrency(requestBody.data.amount.toFixed(2)),
          ...
          var configRecord, credit_memo_pdf_rs;
          if (subsidiaryFeatureCheck) {
            try {
        ...
        try {
          ...
          var recipientEmail = getEmailFromCustomer(customerRec, requestBody.data.location);
          ...
          if (emailContent) {
            ...
            var nameAndAddressArray = getAddressForEmailBody(cmRec);
