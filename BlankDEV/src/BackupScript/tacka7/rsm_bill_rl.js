 ...
 */
define(['N/record', 'N/query', 'N/runtime', 'N/log', './dateUtil.js'],
  function (record, query, runtime, log, dateUtil) {
    ...
    function post(requestBody) {
      if (requestBody.action === 'createJournal') {
        ...
        // Query to get account id of CARINSKA OSNOVICA
        var accountQuery = query.runSuiteQL({
          query: "SELECT id, accountsearchdisplaynamecopy FROM account WHERE accountsearchdisplaynamecopy = ?",
          params: ["CARINSKA OSNOVICA"]
        });
        var accountId = accountQuery.results[0].values[0];
