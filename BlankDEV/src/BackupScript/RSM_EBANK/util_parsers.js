...
function(file, xml, query, runtime) {
	...
	function _parser_halcom(context) {
		...
		// TODO ovde verovatno treba da se referenciramo na neki
		// podataka iz RSM_bstmt ...

		var accountStatementId = "HALCOM-CSV";
	...
	function _lookupPNBO(srchBroj, srchSub) {
		...
		if (subcheck){
			...
			sqlQuery += " where (transaction.type = 'CustInvc') AND (transaction.custbody_poziv_na_broj = ?) ";
			sqlQuery += " AND ( transactionline.subsidiary = ? ) "
			var parQuery = [ srchBroj, srchSub ];
		} else {
		...
		// TODO Resiti problem sta ako ima vise od jedne transakcije
		...
		if ((l_found == 0) && (srchBroj)) {
			...
			if (subcheck){
				var sqlQuery = " select transaction.id, transaction.entity from transactionline join transaction on (transactionline.transaction = transaction.id) ";
				sqlQuery += " where (transaction.type = 'SalesOrd') AND (transaction.custbody_poziv_na_broj = ?) ";
				sqlQuery += " AND ( transactionline.subsidiary = ? ) "
				var parQuery = [ srchBroj, srchSub ];
			} else {
		...
		if ((l_found == 0) && (srchBroj)) {
			...
			if (subcheck){
				...
				sqlQuery += " where (transaction.type like 'CuTrSale%') AND (transaction.custbody_poziv_na_broj = ?) ";
				sqlQuery += " AND ( transactionline.subsidiary = ? ) "
				var parQuery = [ srchBroj, srchSub ];
			} else {
