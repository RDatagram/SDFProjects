/**
 * @NApiVersion 2.1
 * @NScriptType MapReduceScript
 * @NModuleScope SameAccount
 */
define([ 'N/search', 'N/record', 'N/runtime', 'N/query', 'N/format' ],

function(search, record, runtime, query, format) {

	/**
	 * Marks the beginning of the Map/Reduce process and generates input data.
	 * 
	 * @typedef {Object} ObjectRef
	 * @property {number} id - Internal ID of the record instance
	 * @property {string} type - Record type id
	 * 
	 * @return {Array|Object|Search|RecordRef} inputSummary
	 * @since 2015.1
	 */
	function getInputData() {

		// custscript_rsm_bstmt_payments_id
		var scriptObj = runtime.getCurrentScript();
		var masterId = scriptObj.getParameter('custscript_rsm_bstmt_payments_id');
		// custscript_rsm_bstmt_deposit_tax
		// var myOptions = {};

		var sql_from = "select bdp.custrecord_rsm_bdp_customer as v_customer " 
				+ ", bdp.custrecord_rsm_bdp_amount_p as v_amount " 
				+ ", NVL2(bdp.custrecord_rsm_bdp_memo_p,bdp.custrecord_rsm_bdp_memo_p,'') as v_memo "
				+ ", bstmt.custrecord_rsm_bstmt_subsidiary as v_subsidiary " 
				+ ", bstmt.custrecord_rsm_bstmt_bank_acct as v_coabank " 
				+ ", bstmt.custrecord_rsm_bstmt_currency as v_currency "
				+ ", bstmt.custrecord_rsm_bstmt_trandate as v_trandate " 
				+ ", bdp.id as v_bdpid "
				+ ", bdp.custrecord_rsm_bdp_pozivnabroj1 as v_pnbo "
				+ ", bdp.custrecord_rsm_bdp_cs_bank_account as v_cs_bank_account "
				+ ", bdp.custrecord_rsm_bdp_bankref as v_bankref "
				+ ", BUILTIN.DF(custrecord_rsm_bdp_action) as v_action " 
				+ ", jcust.receivablesaccount as v_aracct "
				+ ", (NVL2(bdp.custrecord_rsm_bdp_transaction_recog, bdp.custrecord_rsm_bdp_transaction_recog, -1)) as v_tran_recog "
				+ " from customrecord_rsm_bank_data_parsed bdp join customrecord_snt_bank_statement bstmt on bdp.custrecord_rsm_bdp_parent = bstmt.id " 
				+ " join customer jcust on jcust.id = bdp.custrecord_rsm_bdp_customer "
				+ " where (NVL2(custrecord_rsm_bdp_transaction,'T','F') = 'F' ) AND (bstmt.id = ?)";

		var myPagedResults = query.runSuiteQLPaged({
			pageSize : 1000,
			query : sql_from,
			params : [ masterId ]
		});

		var retArr = [];
		var iterator = myPagedResults.iterator();

		iterator.each(function(resultPage) {
			var currentPage = resultPage.value;
			var theData = currentPage.data.asMappedResults();
			for (var a = 0; a < theData.length; a++) {
				retArr.push(theData[a]);
			}
			return true;
		});

		//log.debug('Array', retArr.length);

		return retArr;
	}

	/**
	 * Executes when the map entry point is triggered and applies to each
	 * key/value pair.
	 * 
	 * @param {MapSummary}  context - Data collection containing the key/value pairs to
	 *            process through the map stage
	 * @since 2015.1
	 */
	function map(context) {

		var userObj = runtime.getCurrentUser();
		var vSystemAR = userObj.getPreference({
			name : 'ARACCOUNT'
		});
		var subcheck = runtime.isFeatureInEffect({
			feature : 'SUBSIDIARIES'
		});

		// log.debug("Step > ", context.value);

		var result = JSON.parse(context.value);

		/**
		 * create customer payments
		 */
		var idCpRec;
		if (result.v_action === 'PAYMENT') {

			try {

				let hasTranRecog = (result.v_tran_recog != -1);

				var toTransform = false;
				if (hasTranRecog) {

					var sql_type = " select type from transaction where id = ? ";
					var results_type = query.runSuiteQL({
						query : sql_type,
						params : [ result.v_tran_recog ]
					});
					var objdata = results_type.asMappedResults();
					for (var vo = 0; vo < objdata.length; vo++) {
						if (objdata[vo]["type"] === 'CustInvc') {
							toTransform = true;
						}
					}
				}

				let cpRec;

				if (toTransform) {
					log.debug('step', 'ToTransform');
					cpRec = record.transform({
						fromType : 'invoice',
						fromId : result.v_tran_recog,
						toType : 'customerpayment',
						isDynamic : true
					});

					cpRec.setValue('undepfunds', 'F');
					cpRec.setValue('account', result.v_coabank);

				} else {
					log.debug('step', 'Create');
					cpRec = record.create({
						type : 'customerpayment',
						isDynamic : true
					});

					cpRec.setValue('customer', result.v_customer);

					if (result.v_aracct == -10) {
						cpRec.setValue('arracct', vSystemAR); // Use system
					} else {
						cpRec.setValue('aracct', result.v_aracct); // A/R
					}

					if (subcheck) {
						cpRec.setValue('subsidiary', result.v_subsidiary);
					}
					cpRec.setValue('currency', result.v_currency);
					cpRec.setValue('undepfunds', 'F');
					cpRec.setValue('account', result.v_coabank);

					cpRec.setValue('autoapply', false);
				}

				cpRec.setValue('trandate', format.parse({
					value : result.v_trandate,
					type : format.Type.DATE
				}));

				cpRec.setValue('payment', result.v_amount);

				/*
					Definisati sta ide u MEMO ? Svrha placanja ili PNBO
					PNBO : zbog Match Bank Data koji koristi MEMO kao PNBO
				 */
				//cpRec.setValue('memo', result.v_memo);
				cpRec.setValue('memo', result.v_pnbo);

				if (result.v_bankref) {
					cpRec.setValue('custbody_rsm_bdp_bankref', result.v_bankref);
					cpRec.setValue('checknum', result.v_bankref);
				}
				
				if (toTransform) {

					var lAmount = cpRec.getCurrentSublistValue({
						sublistId : 'apply',
						fieldId : 'amount'
					});

					var lPayment = cpRec.getValue({
						fieldId : 'payment'
					});

					if (lAmount > lPayment) {
						cpRec.setCurrentSublistValue({
							sublistId : 'apply',
							fieldId : 'amount',
							value : lPayment
						})
					}
				}

				idCpRec = cpRec.save();

				record.submitFields({
					type : 'customerpayment',
					id : idCpRec,
					values : {
						'checknum' : "# " + cpRec.getValue('tranid')
					}
				})
				record.submitFields({
					type : 'customrecord_rsm_bank_data_parsed',
					id : result.v_bdpid,
					values : {
						'custrecord_rsm_bdp_transaction' : idCpRec
					},
					options : {
						enableSourcing : false,
						ignoreMandatoryFields : true
					}
				});
			} catch (e) {
				log.error("Error" + result.v_customer, e.message);
			}
			log.debug('cpRec id', idCpRec);
		}

		if (result.v_action === 'DEPOSIT') {
			var scriptObj = runtime.getCurrentScript();
			var taxCodeId = scriptObj.getParameter('custscript_rsm_bstmt_deposit_tax');

			try {

				let hasTranRecog = (result.v_tran_recog != -1);

				let cpRec = record.create({
					type : 'customerdeposit',
					isDynamic : true
				});

				cpRec.setValue('customer', result.v_customer);

				if (subcheck) {
					cpRec.setValue('subsidiary', result.v_subsidiary);
				}

				cpRec.setValue('currency', result.v_currency);
				cpRec.setValue('trandate', format.parse({
					value : result.v_trandate,
					type : format.Type.DATE
				}));

				cpRec.setValue('account', result.v_coabank);
				cpRec.setValue('payment', result.v_amount);

				cpRec.setValue('memo', result.v_pnbo);
				cpRec.setValue('custbody_poreski_kod_cust_dep_rsm', taxCodeId); // PARAMETAR!!!
				if (result.v_bankref) {
					cpRec.setValue('custbody_rsm_bdp_bankref', result.v_bankref);
				}

				// cpRec.setValue('salesorder', result.v_salesorder);

				if (hasTranRecog) {

					var soField = cpRec.getField({
						"fieldId" : "salesorder"
					});
					var soArray = soField.getSelectOptions();
					log.debug('soArray', soArray);

					let lookUp = search.lookupFields({
						type : search.Type.SALES_ORDER,
						id : result.v_tran_recog,
						columns : ['location']
					});
					log.debug({title:"lookup",details:lookUp});
					let locId = '';
					if (lookUp){
						locId = lookUp.location[0].value;
					}
					log.debug({title:"locId",details:locId});
					if (locId){
						log.debug({title:"Step > ",details:"set CD location"});
						cpRec.setValue({
							fieldId : 'location',
							value : locId
						});
					}

					for (var sio = 0; sio < soArray.length; sio++) {
						if (soArray[sio].value === result.v_tran_recog) {
							cpRec.setValue('salesorder', result.v_tran_recog);
							// Get Location from SalesOrder

							break;
						}
					}
				}

				idCpRec = cpRec.save();

				log.debug('After save', idCpRec);

				record.submitFields({
					type : 'customrecord_rsm_bank_data_parsed',
					id : result.v_bdpid,
					values : {
						'custrecord_rsm_bdp_transaction' : idCpRec
					},
					options : {
						enableSourcing : false,
						ignoreMandatoryFields : true
					}
				});

			} catch (e) {
				log.error({title:"Error " + result.v_customer, details: e.message});
			}
			log.debug('cpRec id', idCpRec);
		}

		if ((result.v_action === 'CS_PAYMENT') || (result.v_action === 'CC_PAYMENT')){
			try {

				var idJeRec;

				var jeRec = record.create({
					type : record.Type.JOURNAL_ENTRY,
					isDynamic : true
				});

				jeRec.setValue({
					fieldId : 'subsidiary',
					value : result.v_subsidiary
				});
				jeRec.setValue({
					fieldId: 'currency',
					value: result.v_currency
				});
				jeRec.setValue('trandate', format.parse({
					value : result.v_trandate,
					type : format.Type.DATE,
					timezone : format.Timezone.EUROPE_BUDAPEST
				}));

				// Credit line - Line 1
				jeRec.selectNewLine({
					sublistId: 'line'
				});
				jeRec.setCurrentSublistValue({
					sublistId: 'line',
					fieldId: 'debit',
					value: result.v_amount
				});
				jeRec.setCurrentSublistValue({
					sublistId: 'line',
					fieldId: 'account',
					value: result.v_coabank
				});
				jeRec.commitLine({
					sublistId: 'line'
				});

				// Credit line - Line 2
				jeRec.selectNewLine({
					sublistId: 'line'
				});
				jeRec.setCurrentSublistValue({
					sublistId: 'line',
					fieldId: 'credit',
					value: result.v_amount
				});
				jeRec.setCurrentSublistValue({
					sublistId: 'line',
					fieldId: 'account',
					value: result.v_cs_bank_account
				});
				jeRec.commitLine({
					sublistId: 'line'
				});
				idJeRec = jeRec.save();
				log.debug('After save', idJeRec);

				record.submitFields({
					type : 'customrecord_rsm_bank_data_parsed',
					id : result.v_bdpid,
					values : {
						'custrecord_rsm_bdp_transaction' : idJeRec
					},
					options : {
						enableSourcing : false,
						ignoreMandatoryFields : true
					}
				});
			} catch (e) {
				log.error({
					title : 'Error during Journal Entry Save',
					details : JSON.stringify(e)
				})
			}
		}
	}

	/**
	 * Executes when the reduce entry point is triggered and applies to each
	 * group.
	 * 
	 * @param {ReduceSummary} context - Data collection containing the groups to process
	 *            through the reduce stage
	 * @since 2015.1
	 */
	function reduce(context) {

	}

	/**
	 * Executes when the summarize entry point is triggered and applies to the
	 * result set.
	 * 
	 * @param {Summary} summary - Holds statistics regarding the execution of a
	 *            map/reduce script
	 * @since 2015.1
	 */
	function summarize(summary) {
		log.audit('inputSummary:Usage', summary.inputSummary.usage);
		log.audit('mapSummary:Usage', summary.mapSummary.usage);
		log.audit('reduceSummary:Usage', summary.reduceSummary.usage);
		log.audit('Usage', summary.usage);
	}

	return {
		getInputData : getInputData,
		map : map,
		reduce : reduce,
		summarize : summarize
	};

});
