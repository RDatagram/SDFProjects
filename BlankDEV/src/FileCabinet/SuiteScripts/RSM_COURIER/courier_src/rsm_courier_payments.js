...
define([ 'N/search', 'N/record', 'N/runtime', 'N/query', 'N/format' ],
    ...
    function(search, record, runtime, query, format) {
        ...
        function getInputData() {
            ...
            var masterId = scriptObj.getParameter('custscript_rsm_courier_payment_mr_id');


            var sql_from = "select csdl.custrecord_rsm_csdl_customer as v_customer "
                + ", csdl.custrecord_rsm_csdl_amount as v_amount "
                + ", NVL2(csdl.custrecord_rsm_csdl_name_pay,csdl.custrecord_rsm_csdl_name_pay,'') as v_memo "
                + ", csdh.custrecord_rsm_csdh_subsidiary as v_subsidiary "
                + ", csdh.custrecord_rsm_csdh_bank_account as v_coabank "
                + ", csdh.custrecord_rsm_csdh_currency as v_currency "
                + ", csdh.custrecord_rsm_csdh_date as v_trandate "
                + ", csdl.id as v_csdlid "
                + ", BUILTIN.DF(custrecord_rsm_csdl_action) as v_action "
                + ", jcust.receivablesaccount as v_aracct "
                + ", (NVL2(csdl.custrecord_rsm_csdl_transaction_recog, csdl.custrecord_rsm_csdl_transaction_recog, -1)) as v_tran_recog "
                + " from customrecord_rsm_csdl csdl join customrecord_rsm_csdh csdh on csdl.custrecord_rsm_csdl_csdh = csdh.id "
                + " join customer jcust on jcust.id = csdl.custrecord_rsm_csdl_customer "
                + " where (NVL2(custrecord_rsm_csdl_transaction,'T','F') = 'F' ) AND (csdh.id = ?)";
        ...
        function map(context) {
                ...
                try {
                    ...
                    if (toTransform) {
                        ...
                        if (result.v_aracct == -10) {
                            ...
                            log.debug({title: "v_arract", details: result.v_arract});
                    ...
                    var id = record.submitFields({
                        ...
                        id : result.v_csdlid,
                        values : {
                            'custrecord_rsm_csdl_transaction' : idCpRec
                        },
                        options : {
                    ...
                    log.error("Error" + result.v_customer, e);
                }
            ...
            if (result.v_action === 'DEPOSIT') {
                var scriptObj = runtime.getCurrentScript();
                var taxCodeId = scriptObj.getParameter('custscript_rsm_courier_deposit_tax');
