...
define(['N/record', 'N/render', 'N/config', 'N/runtime', 'N/file', 'N/url', 'N/email', 'N/log', './dateUtil.js', 'N/query', 'N/search', 'N/util'],
  function (record, render, config, runtime, file, url, email, log, dateUtil, query, search, util) {
    ...
    function getAttachedFiles(recordId, recordType) {
      ...
      while (true) {
        var tempList = SOSearchObj.getRange({
          start: start,
          end: end
        });
    ...
    function post(requestBody) {
      ...
      if (requestBody.action === 'createpdf') {
        ...
        try {
          ...
          for (var i = 0; i < lineCount; i++) {
            ...
            var unitPriceFull = parseFloat(salesOrderRec.getSublistValue({
              sublistId: 'item',
            ...
            var amtIno = 0,
              ...
              grsAmtIno = 0,
              unitPriceFullIno = 0;
            if (soCurrency !== 'RSD' && (customerCountry === 'RS' || customerCountry === 'Serbia')) {
              amtIno = amt;
              taxAmtIno = taxAmt;
              unitPriceIno = (unitPrice) ? unitPrice : 0;
              unitPriceFullIno = (unitPriceFull) ? unitPriceFull : 0;
            ...
            soTaxAmount += taxAmt;
            soAmount += amt;
            var packageQuantity = salesOrderRec.getSublistValue({
              sublistId: 'item',
            ...
            items.push({
              ...
              discount: salesOrderRec.getSublistValue({
                sublistId: 'item',
          ...
          var napomenaOPoreskomOslobadjanju = '',
            ...
            nacinPlacanja = '';
          try {
            var nacinIsporukeId = salesOrderRec.getValue({
          ...
          try {
            var nacinPlacanjaId = salesOrderRec.getValue({
          ...
          data.orderNum = salesOrderRec.getValue('custbody_rsm_crm_ordernum');
          data.transactionCurrency = soCurrency;
          ...
          var configRecord, sales_order_pdf_rs, sales_order_pdf_ino;
          if (subsidiaryFeatureCheck) {
            try {
          ...
          if (custCountry === 'RS' || custCountry === '' || custCountry === 'ME' || custCountry === 'BA' || custCountry === 'HR') {
            ...
            sales_order_pdf_ino = configRecord.getValue({
              fieldId: 'custrecord_rsm_config_so_pdf_ino'
            });
            if (!sales_order_pdf_ino) {
              log.error('Error', 'Sales order PDF template field is empty inside Subsidiary Config.')
              message = {
                type: 'error',
                title: 'Greska!',
            ...
            renderer.setTemplateByScriptId(sales_order_pdf_ino);
          }
      ...
      if (requestBody.action === 'emailpdf') {
        try {
          ...
          var locationText = salesOrderRec.getText('location');
          ...
          var sales_order_email_rs = configRecord.getValue({
            fieldId: 'custrecord_rsm_config_so_email'
          });
          var sales_order_email_ino = configRecord.getValue({
            fieldId: 'custrecord_rsm_config_so_email_ino'
          })

          var emailTemplateId = '';
          if (custCountry === 'RS' || custCountry === '' || custCountry === 'ME' || custCountry === 'BA' || custCountry === 'HR') {
            emailTemplateId = sales_order_email_rs
          } else {
            emailTemplateId = sales_order_email_ino;
          }
          ...
          var attachedFiles = [];
          var attachedFilesId = getAttachedFiles(salesOrderRec.id, salesOrderRec.type);
