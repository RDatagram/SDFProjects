...
define(["N/search", "N/record", "N/file", "N/log"], function (search, record, file, log) {
  ...
  function getAdvancedPaymentTransactions(dateStart, dateEnd, subsidiaryId) {
    ...
    var results = search.create({
      ...
      columns: [
        ...
        'custbody_poreski_kod_cust_dep_rsm.rate',
        'custbody_poreski_kod_cust_dep_rsm.isreversecharge',
    ...
    util.each(ssResults, function (result) {
      obj[result.getValue('internalid')] = {
        ...
        rate: parseInt(result.getValue('custbody_cust_dep_poreska_stopa').replace(/%/g, '')) ||
          parseInt(result.getValue({ name: 'rate', join: 'custbody_poreski_kod_cust_dep_rsm' }).replace(/%/g, '')) || null,
        ...
        rc: result.getValue({ name: 'isreversecharge', join: 'custbody_poreski_kod_cust_dep_rsm' }),
  ...
  function creditMemoJournalEntriesSS(dateStart, dateEnd, subsidiaryId) {
    ...
    var results = search.create({
      ...
      filters: [
        ...
        "AND",
        ['taxitem.custrecord_4110_import', 'is', 'T'],
        'AND',
        ['taxitem.isexport', 'is', 'T'],
        "AND",
        dateFilter
      ],
      columns: [
    ...
    util.each(ssResults, function (result) {
      list.push({
        ...
        taxCode: {
          text: result.getText('taxcode'),
          value: result.getValue('taxcode'),
        },
        grossAmount: parseFloat(result.getValue('grossamount')),
        taxAmount: parseFloat(result.getValue('taxamount')),
  ...
  */
  ...
  function mapPopdvElements(
    ...
    if (field.match(/^1\./)) {
      ...
      if (
        ...
        if (reducedRate) {
          popdvScheme[popdvElement][field][field + ".3"] += Math.abs(
            grossAmount
          );
        } else {
          popdvScheme[popdvElement][field][field + ".1"] += Math.abs(
            grossAmount
          );
        }
      } else {
      ...
      if (deductionField.match(/^6.4/)) {
        ...
        if (
          ...
          if (reverseCharge) {
            ...
            popdvScheme['POPDV8'][deductionField] += (notionalTaxAmount) ? notionalTaxAmount : 0;
          } else {
            popdvScheme['POPDV8'][deductionField] += (taxAmount) ? taxAmount : 0;
          }
        } else {
          popdvScheme["POPDV8"][deductionField] += taxAmount
            ? taxAmount
            : 0;
        }
      }
    ...
    if (reverseCharge && reverseChargeField.match(/^3a/)) {
      ...
      if (reducedRate) {
        popdvScheme["POPDV3a"][reverseChargeField][reverseChargeField + ".2"] += notionalTaxAmount ? notionalTaxAmount : 0;
      } else {
        popdvScheme["POPDV3a"][reverseChargeField][reverseChargeField + ".1"] += notionalTaxAmount ? notionalTaxAmount : 0;
      }
    }
  }
  ...
  function calculateSumValues(popdvScheme) {
    ...
    // Check for 3.1 field
    for (var i = 1; i <= 4; i++) {
      popdvScheme['POPDV3']['3.1']['3.1.' + i] = (popdvScheme['POPDV3']['3.1']['3.1.' + i] < 0) ? 0 : popdvScheme['POPDV3']['3.1']['3.1.' + i];
    }
    // Check for 3.2 field
    for (var i = 1; i <= 4; i++) {
      popdvScheme['POPDV3']['3.2']['3.2.' + i] = (popdvScheme['POPDV3']['3.2']['3.2.' + i] < 0) ? 0 : popdvScheme['POPDV3']['3.2']['3.2.' + i];
    }
    // Check for 3.7 field
    for (var i = 1; i <= 4; i++) {
      popdvScheme['POPDV3']['3.7']['3.7.' + i] = (popdvScheme['POPDV3']['3.7']['3.7.' + i] < 0) ? 0 : popdvScheme['POPDV3']['3.7']['3.7.' + i];
    }
    ...
    for (var i = 1; i <= 2; i++) {
      ...
      popdvScheme["POPDV4"]["4.1.3"]["4.1.3." + i] = (tempValue < 0) ? 0 : tempValue;
    }
    for (var i = 1; i <= 2; i++) {
      tempValue =
        popdvScheme["POPDV4"]["4.2.1"]["4.2.1." + i] -
        popdvScheme["POPDV4"]["4.2.2"]["4.2.2." + i];
      popdvScheme["POPDV4"]["4.2.3"]["4.2.3." + i] = (tempValue < 0) ? 0 : tempValue;
    }
    // Table 5 sums
    ...
    // 6.4 check
    popdvScheme["POPDV6"]['6.4'] = (popdvScheme["POPDV6"]['6.4'] < 0) ? 0 : popdvScheme["POPDV6"]['6.4'];
    ...
    if (popdvScheme["POPDV8"]["8e.6"] < 0) {
      popdvScheme["POPDV5"]["5.3"] =
        popdvScheme["POPDV5"]["5.2"] +
        Math.abs(popdvScheme["POPDV8"]["8e.6"]);
    } else {
    ...
    // After calculation of 8e.6 field, check if 5.2 and 5.5 are negative. If they are, set their value to 0
    popdvScheme["POPDV5"]["5.2"] = (popdvScheme["POPDV5"]["5.2"] < 0) ? 0 : popdvScheme["POPDV5"]["5.2"];
    popdvScheme["POPDV5"]["5.5"] = (popdvScheme["POPDV5"]["5.5"] < 0) ? 0 : popdvScheme["POPDV5"]["5.5"];
    popdvScheme["POPDV8"]["8e.6"] = (popdvScheme["POPDV8"]["8e.6"] < 0) ? 0 : popdvScheme["POPDV8"]["8e.6"];
    ...
    // Table 9a Sum
    popdvScheme["POPDV9a"]["9a.1"] = (popdvScheme["POPDV6"]["6.4"] < 0) ? 0 : popdvScheme["POPDV6"]["6.4"];
    popdvScheme["POPDV9a"]["9a.2"] = (popdvScheme["POPDV7"]["7.4"]["7.4.2"] < 0) ? 0 : popdvScheme["POPDV7"]["7.4"]["7.4.2"];
    popdvScheme["POPDV9a"]["9a.3"] = (popdvScheme["POPDV8"]["8e.6"] < 0) ? 0 : popdvScheme["POPDV8"]["8e.6"];
    ...
    // Table/Field 10 diff
    popdvScheme["POPDV10"] = popdvScheme["POPDV5"]["5.7"] - popdvScheme["POPDV9a"]["9a.4"];
  }
  ...
  */
  ...
  function processAdvancedPayments(popdvScheme, transactions, taxCodeObj) {
    util.each(transactions, function (tran, id) {
      ...
      var parentRate =
        tran.rc && taxCodeObj[tran.taxCode]["parent"]
          ? record
            .load({
              id: taxCodeObj[tran.taxCode]["parent"],
              type: search.Type.SALES_TAX_ITEM
            })
            .getValue("rate")
          : null;
        ...
        for (var field in popdvScheme[popdvEl]) {
          if (typeof popdvScheme[popdvEl][field] === "object" && popdvScheme[popdvEl][field] !== null) {
            for (var finalField in popdvScheme[popdvEl][field]) {
              popdvScheme[popdvEl][field][finalField] = Math.round(popdvScheme[popdvEl][field][finalField]);
            }
          } else {
            popdvScheme[popdvEl][field] = Math.round(popdvScheme[popdvEl][field]);
          }
        }
      } else {
  ...
  function populatePopdvScheme(popdvScheme, dateStart, dateEnd, subsidiaryId) {
    ...
    popdvData.each(function (result) {
      var taxCode = result.getValue({ name: 'taxcode', summary: search.Summary.GROUP });
      // var grossAmount = parseFloat(result.getValue({ name: "grossamount", summary: search.Summary.SUM }));
      var grossAmount = parseFloat(result.getValue({ name:"formulacurrency", summary: search.Summary.SUM}));
      var taxAmount = parseFloat(result.getValue({ name: "taxamount", summary: search.Summary.SUM }));
      ...
      // Get parent rate if tax code is reverse charge
      var parentRate =
        reverseCharge && taxCodeObj[taxCode]["parent"]
          ? record
            .load({
              id: taxCodeObj[taxCode]["parent"],
              type: search.Type.SALES_TAX_ITEM
            })
            .getValue("rate")
          : null;
