 ...
 * @NModuleScope Public
 * 
 * Custom module with IoOR calculation functionality. 
 * 
...
define(["N/search", "N/file", "N/render", "./dateUtil", "./incomeStatement"], function (search, file, render, dateUtil, incomeStatement) {
    ...
    function loadScheme() {
        ...
        ioorScheme = JSON.parse(file1.getContents());
    }
    ...
    function setBalancesAccounts(obj, obj2) {
        for (var keyOne in ioorScheme) {
            if (ioorScheme[keyOne].hasOwnProperty("accounts")) {
                var sum = 0, sum2 = 0;
                var accList = ioorScheme[keyOne]["accounts"];
                ...
                ioorScheme[keyOne]["current_year"] = sum;
                ioorScheme[keyOne]["last_year"] = sum2;
            }
        }
    }
    ...
    function setBalancesAops() {
        for (var key in ioorScheme) {
            if (ioorScheme[key].hasOwnProperty("aops")) {
                var sum = 0, sum2 = 0;
                var aops = ioorScheme[key]["aops"];
                for (var i = 0; i < aops.length; i++) {
                    var result = aops[i].split(/\s/);
                    var sign = result[0], aop = result[1];
                    if (sign === "+") {
                        sum += ioorScheme["_" + aop]["current_year"];
                        sum2 += ioorScheme["_" + aop]["last_year"];
                    } else {
                        sum -= ioorScheme["_" + aop]["current_year"];
                        sum2 -= ioorScheme["_" + aop]["last_year"];
                    }
                ...
                ioorScheme[key]["current_year"] = sum;
                ioorScheme[key]["last_year"] = sum2;
            }
        }
    }

    // Round values in ioor_scheme
    function roundValues() {
        for (var key in ioorScheme) {
            ioorScheme[key]["current_year"] = Math.round(ioorScheme[key]["current_year"]);
            ioorScheme[key]["last_year"] = Math.round(ioorScheme[key]["last_year"]);
        }
    }
    ...
    // Create xml string with fields and values from ioorScheme
    function createXMLString() {
        ...
        for (var key in ioorScheme) {
            ...
            numerickoPolje += "<a:Vrednosti>" + (ioorScheme[key]["current_year"]).toString() + "</a:Vrednosti>";
            ...
            numerickoPolje += "<a:Vrednosti>" + (ioorScheme[key]["last_year"]).toString() + "</a:Vrednosti>";
            numerickoPolje += "</a:NumerickoPolje>";
    ...
    // Calculate data, populate ioor_scheme object
    function processData(periodFrom, periodTo) {
        loadScheme();

        // Get incomeStatement aops 1064 and 1065 (current and last year)
        var incomeStatementAops = incomeStatement.getIoorAops(periodFrom, periodTo);
        ioorScheme["_2001"].current_year = incomeStatementAops["current_year_1064"];
        ioorScheme["_2001"].last_year = incomeStatementAops["last_year_1064"];
        ioorScheme["_2002"].current_year = incomeStatementAops["current_year_1065"];
        ioorScheme["_2002"].last_year = incomeStatementAops["last_year_1065"];
    ...
    function savePdf(periodFrom, periodTo, maticniBroj, sifraDelatnosti, pib) {
        ...
        renderer.setTemplateByScriptId("CUSTTMPL_IOOR_PDF_HTML_TEMPLATE");
        var pdfFile = renderer.renderAsPdf();
        pdfFile.name = "Izvestaj o ostalom rezultatu-" + datumOd + "-" + datumDo + ".pdf";
        return pdfFile;
    }
