...
define(['N/log', 'N/query'], function (log, query) {
  ...
  function getLineCount(recordId) {
    var sublistQuery = query.runSuiteQL({
      query: 'SELECT COUNT(*) FROM customrecord_rsm_ior_report_lines WHERE custrecord_rsm_iorl_report_parent = ?',
      params: [recordId]
    });
  ...
  function beforeLoad(scriptContext) {
    if (scriptContext.type === scriptContext.UserEventType.VIEW) {
      ...
      var pdfFileId = record.getValue("custrecord_rsm_ior_report_pdf_file");
      var xmlFileId = record.getValue("custrecord_rsm_ior_report_xml_file");
      var xlsFileId = record.getValue("custrecord_rsm_ior_report_xls_file");
      var lineCount = getLineCount(record.id);


      form.clientScriptModulePath = './cs_ior_report.js';
      form.addButton({
        id: 'custpage_btn_init_lines',
        label: 'Init lines',
        functionName: 'init_ior_lines'
      });
      if (lineCount > 0) {
        form.addButton({
      ...
      form.addButton({
        ...
        label: 'Calculate',
        functionName: 'calculate_ior_lines'
      });
      form.addButton({
