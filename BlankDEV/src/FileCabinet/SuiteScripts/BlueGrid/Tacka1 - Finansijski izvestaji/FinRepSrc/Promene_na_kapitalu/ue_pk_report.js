...
define(['N/log', 'N/query'], function(log, query) {
  ...
  function getLineCount(recordId) {
    var sublistQuery = query.runSuiteQL({
      query: 'SELECT COUNT(*) FROM customrecord_rsm_pk_report_lines WHERE custrecord_rsm_pkl_report_parent = ?',
      params: [recordId]
    });
  ...
  function beforeLoad(scriptContext) {
    if (scriptContext.type === scriptContext.UserEventType.VIEW) {
      ...
      var pdfFileId = record.getValue("custrecord_rsm_pk_report_pdf_file");
      var xmlFileId = record.getValue("custrecord_rsm_pk_report_xml_file");
      var xlsFileId = record.getValue("custrecord_rsm_pk_report_xls_file");
      var lineCount = getLineCount(record.id);


      form.clientScriptModulePath = './cs_pk_report.js';
      form.addButton({
        id: 'custpage_btn_init_lines',
        label: 'Init lines',
        functionName: 'init_pk_lines'
      });
      if (lineCount > 0) {
        form.addButton({
      ...
      form.addButton({
        ...
        label: 'Calculate',
        functionName: 'calculate_pk_lines'
      });
      form.addButton({
