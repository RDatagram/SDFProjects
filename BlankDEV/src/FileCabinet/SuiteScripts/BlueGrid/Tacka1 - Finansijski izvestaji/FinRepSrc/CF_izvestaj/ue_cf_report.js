...
define(['N/log', 'N/query'], function(log, query) {
  ...
  function getLineCount(recordId) {
    var sublistQuery = query.runSuiteQL({
      query: 'SELECT COUNT(*) FROM customrecord_rsm_cf_report_lines WHERE custrecord_rsm_cf_report_parent = ?',
      params: [recordId]
    });
  ...
  function beforeLoad(scriptContext) {
    if (scriptContext.type === scriptContext.UserEventType.VIEW) {
      ...
      var pdfFileId = record.getValue("custrecord_rsm_cf_report_pdf_file");
      var xmlFileId = record.getValue("custrecord_rsm_cf_report_xml_file");
      var xlsFileId = record.getValue("custrecord_rsm_cf_report_xls_file");

      form.clientScriptModulePath = './cs_cf_report.js';
      ...
      form.addButton({
        ...
        label: 'Calculate',
        functionName: 'calculate_cf_lines'
      });
      form.addButton({
