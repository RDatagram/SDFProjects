...
define(['N/ui/serverWidget', 'N/search', 'N/file', 'N/render', 'N/encode', 'N/log', 'N/ui/message', 'N/http', 'N/util'],
  ...
  function (serverWidget, search, file, render, encode, log, message, http, util) {
    ...
    function ipgFinalToObj(searchResults) {
      ...
      log.debug('RESULTS FINAL', results);
    ...
    function mergeObjects(ipgFinal, ipgBillVariance, ipgTroskoviIsporuke) {
      ...
      for (var key in row) {
        ...
        rows.push(row[key]);
        log.debug('ROW:', JSON.stringify(row[key]));
      }
      return rows;
    }
    ...
    function onRequest(context) {
      ...
      log.debug('Final:', JSON.stringify(ipgFinal));
      log.debug('Bill variance:', JSON.stringify(ipgBillVariance));
      log.debug('Troskovi isporuke:', JSON.stringify(ipgTroskoviIsporuke));
